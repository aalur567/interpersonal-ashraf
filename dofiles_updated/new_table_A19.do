...
gen low=0
replace low=1 if ability_factor<-.17
replace low=. if ability_factor==.
...
xi: reghdfe gu_c11_male_sib neg_above neg_below  safespace `first' `second'  if treatment_actual!=0, absorb(classid) `se'
est store reg1
...
xi: reghdfe gu_c12_female_sib neg_above neg_below  safespace `first' `second' if treatment_actual!=0, absorb(classid) `se'
est store reg2
...
foreach sib in male female {
	...
	xi: reghdfe gu_b2_chores_`sib' neg_above neg_below  safespace `first' `second'  if treatment_actual!=0, absorb(classid) `se'
	est store reg3_`sib'
...
foreach sib in male female {
	...
	xi: reghdfe gu_b3_schoolwork_`sib' neg_above neg_below  safespace `first' `second'  if treatment_actual!=0, absorb(classid) `se'
	est store reg4_`sib'
...
xi: reghdfe gu_e9_fees_allocate_2a neg_above neg_below  safespace `first' `second' if treatment_actual!=0, absorb(classid) `se'	
est store reg6
...
xi: reghdfe gu_f6_school_will_male neg_above neg_below  safespace `first' `second'  if treatment_actual!=0, absorb(classid) `se'
est store reg7
...
xi: reghdfe gu_f7_school_will_female neg_above neg_below  safespace `first' `second'  if treatment_actual!=0, absorb(classid) `se'
est store reg8
...
xml_tab reg1 reg2 reg3_female reg3_male reg4_male reg4_female  reg6 reg7 reg8, save("tables/table_A19.xml") replace sheet("Siblings") below ///
	...
	"Schooling Complete, Male" "Schooling Complete, Female") keep(neg_above neg_below  safespace) 
